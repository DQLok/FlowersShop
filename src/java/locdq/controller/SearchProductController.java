...
public class SearchProductController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url=ERROR+"?product=on";
        try  {
           String SearchProducts=request.getParameter("SearchProducts");
            ProductDAO dao=new ProductDAO();
            List<ProductDTO> listpr=dao.getlistproduct(SearchProducts);
            HttpSession session=request.getSession();
                session.setAttribute("LIST_PRODUCT", listpr);
            if (!listpr.isEmpty()){                
                List<ProductDTO> a=(List<ProductDTO>)session.getAttribute("LIST_PRODUCT");
                url=SUCCESS+"?product=on";
            }
        }catch(Exception e){
            log("Error SearchProduct"+e.toString());
        }finally{
